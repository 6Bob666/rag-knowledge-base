...
```text
上传文件
  → 计算 SHA-256 内容哈希，命中（文件名 + 内容）则直接返回 unchanged
  → 按文件名加锁，串行化同一文件的并发上传
  → 生成 document_id
  → 切分为多个 chunk
...
追问「幂等为什么要用内容哈希，不直接用文件名」：文件名会重复（两个人传
`报告.docx`），也会变（改名后内容没变），只有内容能代表"这是不是同一份东西"。

追问「并发上传同一个文件会怎样」：两个请求的"先查有没有、再决定写不写"会交错，
可能双双入库、甚至把对方刚写的版本当成旧版本删掉。所以临界区要按文件名加锁，
并且锁的粒度是单个文件而不是全局——不同文件之间不该互相排队。

追问「加了字段为什么还要写迁移」：`Base.metadata.create_all` 只建缺失的表，不会
给已存在的表加列。老库直接跑新代码，第一次上传就会因为找不到 `content_hash`
报错。这里用 `ensure_schema()` 做一次幂等的轻量迁移，生产环境应换成 Alembic。
...
`benchmark_index.py` 只测索引层：同一批向量换 HNSW 参数，看 Recall@10 与延迟。

实测（dim=512，top_k=10，intrinsic_dim=32）：

```text
          hnsw_default      hnsw_small_ef     exact_numpy
5 千      1.000 / 1.21ms    0.785             1.000 / 0.18ms
5 万      0.960 / 1.62ms    0.650             1.000 / 3.48ms
20 万     0.950 / 1.54ms    0.455             1.000 / 11.59ms
```

四条结论：ANN 的价值是延迟不随规模涨；`search_ef` 配错会让一半答案进不了候选池；
漏召回不可逆（Reranker 只能精排召回结果，所以 ANN 的 Recall 是整条链路的上限）；
20 万以内暴力检索仍然能打，换 Milvus/FAISS 的真正理由是规模继续上升或要多副本。

追问「压测为什么用合成向量」：只有合成数据能控制难度。关键变量是内在维度——
同一份代码，intrinsic=32 时 20 万规模召回 0.950，intrinsic=512 时只有 0.230。
**脱离数据分布谈召回没有意义**，这是看 ANN 报告时最先该问的问题。

追问「这个压测对你的项目意味着什么」：现在是几百个 chunk，Chroma 默认参数完全
够用，所以我没有为了"上规模"去换 Milvus；真正需要动作的阈值是上面的拐点。
说清楚"什么时候不该换"，比换个组件更难也更有说服力。
